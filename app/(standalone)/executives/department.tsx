...
import { fetchDepartmentExecutives } from '@/api/executives';
...
export default function DepartmentExecutives() {
  ...
  const program = studentUser?.program ?? '';
  ...
  useEffect(() => {
    ...
    fetchDepartmentExecutives(program)
      .then(setExecutives)
      .catch(console.error)
    ...
    <View style={styles.root}>
        ...
        <View style={styles.headerRow}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.headerSub} numberOfLines={1}>
              {program ? `Meet Your · ${program}` : 'Meet Your Department'}
            </Text>
            <Text style={styles.headerTitle}>Department Executives</Text>
          </View>
        </View>
      </LinearGradient>
      ...
      {loading ? (
        ...
        <View style={styles.empty}>
          ...
          <Text style={styles.emptyText}>No department linked to your profile</Text>
        </View>
      ) : (
        <FlatList
          data={executives}
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              ...
              <Text style={styles.emptyText}>No executives listed for{'\n'}{program}</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  headerSub:   { fontSize: 11, color: 'rgba(255,255,255,0.5)', fontWeight: '600', letterSpacing: 0.3 },
